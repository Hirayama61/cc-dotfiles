...
ただしこれは Claude への指示であって強制ではない。停止時ナッジは実行を保証せず、
指示に従わないまま停止することはありうる。

自動更新が走らなかった場合(指示が実行されなかった、usage.json が無い・古い、
構造検証に通らない等)は、`/compact` を打った時点で PreCompact ゲートが 1 回止めて
compact-prep を要求する。
