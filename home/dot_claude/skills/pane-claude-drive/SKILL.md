...
4. **指示を注入**: literal 指示に、**そのタスクのタスク指示書のパスだけ**を渡し(handoff doc 全文は
   ...
   **「完了レポートの冒頭行にも同じ nonce を書く」ことを指示に含める**(§7 の完了条件が
   ...
   nonce を書かず完了検知に落ちる)。
   **完了レポートの長さも指示に含める** — やったこと・判断・懸念・成果物パスをタスクが
   要する分だけ書き、埋め草の節・重複した要約で水増ししない(長さを指示しないと
   被運転のレポートは肥大し、レポート駆動検品(§6)のコンテキスト最適化が痩せる)。承認範囲の区別(**許可済みなのはこのタスク自体で、
   ...
   **タスク指示書の必須項目**(被運転が読む唯一の入力。口伝にしない):
   ...
   | 完了条件 | 完了ファイルとレポートの出力先、満たすべき状態。レポートはタスクが要する分だけの長さ(水増ししない) |
