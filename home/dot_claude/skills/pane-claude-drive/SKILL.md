...
- **送信前の権限プロンプト検知は機械的に行う**(モデルの目視判断に依存しない)。現場監督が
  pane へ何かを送る前(代理応答・自動連鎖投入・再開フレーズのいずれも)に、`capture-pane` の
  **非空行の末尾 25 行**を `PERMISSION_ERE` と照合し、**一致したら送らずに人間へ上げる**。
  窓を非空行で取るのは、`capture-pane -p | tail -N` が末尾の空行に埋もれて空を返すことが
  あるため — 空文字は `grep` を必ず不一致(rc=1)= 唯一の続行値にするので、そのまま送ると
  ...
  # 空行を落としてから窓を取る。tail を先に当てると末尾の空行だけを掴んで空文字になり、
  # 下の照合が「不一致 = 送ってよい」に化ける。
  grc=0; nonblank="$(printf '%s\n' "$tail_txt" | grep -v '^[[:space:]]*$')" || grc=$?
  case "$grc" in
    0|1) ;;
  ...
  非空行 0 のいずれでも送らない)。検知窓の 25 行は rate-limit-resume.sh 本体・
  tmux-claude-drive 手順 3 と同幅。
- **`ListAgents` の状態が `waiting` の pane へは送らない**。上の ERE 照合を通した後の
  ...
  **ERE 照合の代替にはしない** — 検知漏れを減らす方向にしか効かない位置に置く。
  `ListAgents` が使えない / 当該 pane の行が無い環境ではこの壁が無いものとして扱ってよい
  (主壁は ERE 照合の側)。
...
4. **指示を注入**(投入経路は tmux-claude-drive 手順 2): 指示文には**そのタスクのタスク指示書の
   ...
   **cron / loop を作らないことも指示に含める**(tmux-claude-drive 手順 2)。作業者が残した
   cron / loop は勝手に再稼働するうえ、`ListAgents` の状態からは有無が見えないので §10 の
   閉じてよい判定では捕まえられない。
   ...
   **タスク指示書の必須項目**(被運転が読む唯一の入力。口伝にしない):
   ...
   | 絶対に守る制約 | repo 規約・触ってはいけない場所・禁止操作(**cron / loop の作成禁止**を含む) |
...
- **タスク状態機械(schema)**: 各タスク `key` に次を持つ。
  ...
  - `peer_name` / `peer_ref`(`ListAgents` が返す作業者セッションの名前と、同名衝突時に要る
    `[ref]`)/ `reply_addr`(作業者から返信が来た時の `from`。名前ではなく uds ソケットパス)。
    peer 名は `<cwd basename>-<ランダム2文字>` で**予測できない**ので、組み立てずに投入時の
    pane id 照合(tmux-claude-drive 手順 2)で得た値をここへ記録する。記録が無いと、返信や
    追加投入をどのタスクのものか紐付けられない。
...
- **閉じてよいかは 4 条件で判定する**(tmux-claude-drive 手順 5 が正典。ここは案件側の当てはめ):
  `ListAgents` に当該 `pane_id` の行がある ∧ その行の状態が `idle`(`busy` / `shell` /
  `waiting` と、実測されていない状態はすべて不可)∧ 完了ファイルの nonce 一致 ∧ §7 の完了
  フェンス通過。1 つでも欠けたら閉じずに現状を人間へ上げる。
  **`ListAgents` からは cron / loop の有無が見えない**ので、この判定では塞げない
  (塞ぐのは §5-4 の指示側)。**pane の自動 kill はしない** — 判定材料を出すところまでで、
  閉じる操作は現場監督が行う。
