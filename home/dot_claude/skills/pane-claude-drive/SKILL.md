...
- **送信前の権限プロンプト検知は機械的に行う**(モデルの目視判断に依存しない)。現場監督が
  ...
  **末尾 25 行**を `PERMISSION_ERE` と照合し、**一致したら送らずに人間へ上げる**。ERE は
  転記も grep 抽出もせず、公開口から取得する(正典の複製を作らない):
...
3. **補間値のサニタイズ(必須)**: 被運転へ literal 送信する値(fleet-id・taskkey・nonce・
   ...
   `[A-Za-z0-9._-]` へ正規化する(改行混入は send-keys `-l` の途中確定=
...
4. **指示を注入**: literal 指示に、**そのタスクのタスク指示書のパスだけ**を渡し(handoff doc 全文は
   ...
   **タスク指示書の必須項目**(被運転が読む唯一の入力。口伝にしない):
   ...
   | 絶対に守る制約 | repo 規約・触ってはいけない場所・禁止操作 |
