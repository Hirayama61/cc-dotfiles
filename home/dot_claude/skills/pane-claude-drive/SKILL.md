...
4. **指示を注入**: literal 指示に、**そのタスクのタスク指示書のパスだけ**を渡し(handoff doc 全文は
   ...
   **完了レポートの長さも指示に含める** — 埋め草の節・重複した要約で水増ししない。
   短くするのは説明の文章量だけで、やったこと・判断・懸念・成果物パス・未完了項目・
   `triage:` の 1 件 1 行の記録は長さを理由に省略しない(ここが削れると、レポートしか
   読まない検品(§6)から欠落が見えなくなる)。
   ...
   **タスク指示書の必須項目**(被運転が読む唯一の入力。口伝にしない):
   ...
   | 完了条件 | 完了ファイルとレポートの出力先、満たすべき状態。レポートは「やったこと・判断・懸念・成果物パス・未完了項目・`triage:` 行」を必ず含め、長さはタスクが要する分だけ(水増ししない) |
