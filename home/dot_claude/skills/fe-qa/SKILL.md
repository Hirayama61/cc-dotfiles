...
description: >-
  ...
  Figma との視覚照合・レスポンシブ・仕様適合(Confluence 突合)・
...
| 4 | 視覚照合 | 変更画面ごとに Figma の見本と実装のスクリーンショットを突き合わせる(手順は表の下)。Figma 参照なし / 見本を取得できない / 撮影できない なら skip(理由) |
...
1. **見本を取る**。Figma MCP の画像エクスポートを試し、取れなければ人間に見本の添付を依頼する。
   どちらも不可なら skip(見本なし)として記録し、他レンズへ進む。
2. **実装を撮る**。同じ画面・同じビューポートで `browser_take_screenshot` を撮る。
   **照合はテストデータ表示状態で行う** — 実データ・認証情報・個人情報が写る場合は撮影前に
   人間へ確認する(画像は `~/.cache/claude-qa/screenshots/` に残り、伏せ字が効かない)。
3. **項目別に突き合わせる**: 色 / 余白 / タイポグラフィ(サイズ・字間・行間・ウェイト)/ 整列 /
   形状(角丸・影・ボーダー幅・アイコン)。全体を眺めて「だいたい合っている」で終わらせない。
4. **Figma MCP の読取値は、この照合が済むまで未検証扱い**とする(MCP が返す数値と実際の
   レンダリングは一致しないことがあるため、値だけを根拠に「一致」と判定しない)。
5. **差分を軽微だからと自己判断で握らない**。残す差分は finding として上げ、人間の ack を取る。
