...
**window の作り方・pane の選び方・完了合図(nonce と完了ファイル)の決め方・id 照合の作法・
trust ダイアログの扱いはここに書かない。**`tmux-claude-drive`(起動・投入・監視・完了判定)と
`pane-claude-drive`(window 内の pane 並列)に完全委譲し、必要になったらそちらを読む。
相方は俯瞰が仕事で、運転の実務知を二重に持つと鮮度の維持コストが俯瞰を食う。
