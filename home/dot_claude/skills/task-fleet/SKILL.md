...
2. **被運転を起動**: `claude --model opus`(permission-mode 指定なし = defaultMode=auto)。
   ...
   **§1 の送信前 `PERMISSION_ERE` 照合はこの trust ダイアログにも一致する**(`do you trust` を
   ...
   それ以外の一致は例外なく人間へ上げる。例外が成立するのは次の 2 条件が**揃った時だけ**で、
   片方でも欠けたら人間へ上げる:
   - **当該タスクの `status` が `pending`**(= まだ被運転を起動していない)。§7 schema の
     `status` だけを読み、pane の見た目や記憶で判断しない。
   - **capture に `do you trust` が(大小無視で)含まれる**。
     `printf '%s' "$cap" | grep -qi 'do you trust'` を別途実行して判定する
     (`has_permission` は `grep -qiE` で真偽しか返さないため、どの alternative に
     一致したかは取れない)。
   `PERMISSION_ERE` は `[y/n]` 等の他の形も拾うので、`status` だけで縛ると起動前 pane に
   残っていた別の一致まで例外に入る。
...
- **期待成果物はタスク種別ごとに定める**。分割計画で各タスクに `kind` を与え(§7 の schema に
  ...
  **この 3 条件が防ぐのは「前回成果物の使い回し・タスク取り違え」という事故であって、偽装ではない**。
  ...
  ただし比較の粒度は bash 3.2 の `test -nt` が持つ秒単位で、同一秒内に置かれた成果物は
  「新しくない」と判定される(fail-closed 側)。`executor: manager` の短いタスクは投入と
  成果物書き出しが同一秒に入りうるので、この判定で止まった時はまず粒度を疑う。
