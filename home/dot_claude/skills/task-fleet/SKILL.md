...
for wt in "${task_worktrees[@]}"; do
  ...
  # noclobber で atomic に作る(symlink/FIFO も O_CREAT|O_EXCL で弾ける)。
  ( set -C; printf 'fan-out from design-review of %s at %s\n' "$fleet_id" "$(date '+%Y-%m-%d %H:%M')" > "$f" ) \
    || { echo "フラグ展開に失敗: $branch。中断" >&2; exit 1; }

  # Tier 3 スコープもブランチごとに書く(タスク指示書の成果物宣言をそのまま並べる)。
  s="$("$HOME/.claude/hooks/lib/flag-paths.sh" design-scope "$repo" "$branch")"
  [ -L "$s" ] && { echo "scope パスが symlink: $s。中断" >&2; exit 1; }
  printf '%s\n' "${task_scope[@]}" > "$s" \
    || { echo "scope 書込に失敗: $branch。中断" >&2; exit 1; }
done
```
...
- **計画外ブランチへは展開しない**。展開対象は分割計画に列挙し、かつ人間が確認した worktree に
  ...
  Gate 2 に止まり原因が追えない)。
- **design-scope はブランチごとに書く**。`task_scope` はそのタスクの指示書が宣言した成果物の
  path / glob を 1 行 1 件で並べたもの(glob は bash の case パターン形式。ディレクトリは
  `dir/*`)。fan-out で `design-reviewed` だけ立てて scope を落とすと、self-review の Tier 3 が
  そのブランチで照合材料を失う(Tier 3 は surface-only の所見表示で push を止めないため、
  落としても気づかないまま宣言外の変更が積まれる)。
...
- **nonce と完了検知**: fleet+task ごとに管理セッションが起動時に一意採番(再利用しない)。完了
  ...
  (指示エコーの自己反射誤検知を避ける)。nonce 一致だけを完了の根拠にせず、**完了レポートが
  指す成果物 path の存在と、そのタスクブランチに diff があること**を併せて確認する(§8 の機械的
  最低検品を完了検知側へ前倒しする)。
...
- **失敗 pane は kill せず残す**(forensics)。ただし残すのは**スクロールバックだけ**で、被運転
  ...
  pane 上限には数えない)。`capture-pane -J -p -S -2000 -t <pane_id>` で
