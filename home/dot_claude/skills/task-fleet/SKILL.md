...
# fan-out は Gate 2 を N 本まとめて解除する。元の案件設計が実際に design-review を通ったことを
# 展開前に機械確認する(通っていない設計を N 本へ配ると読み違い 1 つで自己解除になる)。
# repo キーは論理 repo 単位なので、同一 repo の worktree どれから引いても同じ値になる。
design_source_branch=""                        # ← 呼び出し側が §7 の design_source_branch で埋める
[ -n "$design_source_branch" ] \
  || { echo "design_source_branch が未設定。設計レビュー済みを確認できないので中断" >&2; exit 1; }
is_protected_branch "$design_source_branch" \
  && { echo "design_source_branch が保護ブランチ: ${design_source_branch}。branch 版フラグを持てないので中断" >&2; exit 1; }
src_repo="$("$HOME/.claude/hooks/lib/resolve-repo-key.sh" "${task_worktrees[0]}" 2>/dev/null || true)"
[ -n "$src_repo" ] || { echo "repo key が引けない: ${task_worktrees[0]}。中断" >&2; exit 1; }
src_flag="$("$HOME/.claude/hooks/lib/flag-paths.sh" design-reviewed "$src_repo" "$design_source_branch")"
[ -n "$src_flag" ] || { echo "フラグパスが引けない: ${design_source_branch}。中断" >&2; exit 1; }
[ -f "$src_flag" ] \
  || { echo "元設計が design-review 未通過: ${design_source_branch}。fan-out しない。中断" >&2; exit 1; }
...
for wt in "${task_worktrees[@]}"; do
  ...
  # あると素の > はリンク先へ書いてしまう)。
  ( set -C; printf 'fan-out from design-review of %s at %s\n' "$fleet_id" "$(date '+%Y-%m-%d %H:%M')" > "$tmp" ) \
...
- **fan-out 前に元設計の解錠を機械確認する**。`design_source_branch`(§7 の案件レベル項目)の
  `design-reviewed` フラグが実在しない限り 1 本も展開しない。「案件設計は着手前に design-review に
  一度通す」を散文の前提のままにすると、読み違い 1 つで Gate 2 を N 本まとめて自己解除できる。
...
2. **被運転を起動**: `claude --model opus`(permission-mode 指定なし = defaultMode=auto)。
   ...
   **§1 の送信前 `PERMISSION_ERE` 照合はこの trust ダイアログにも一致する**(`do you trust` を
   拾う設計)。**この機械ガードの唯一の例外が新規 worktree 初回起動時の trust ダイアログ**で、
   それ以外の一致は例外なく人間へ上げる。例外の適用条件は「当該 pane でまだ被運転を起動して
   いない」ことに限り、起動後に出た一致は trust ダイアログの見た目でも人間へ上げる
   (例外を pane の状態で縛らないと、機械ガード全体が文面判断に戻る)。
...
- **案件レベルの項目**: `design_source_branch`(案件設計の `/design-review` を通した**非保護**
  ブランチ名。§4 のフラグ fan-out はループ前にこのブランチの `design-reviewed` フラグ実在を
  確認し、無ければ中断する。fan-out は Gate 2 を N 本まとめて解除する機構なので、
  「設計レビューを通した」を散文の前提でなく機械確認にする)。
...
- **期待成果物はタスク種別ごとに定める**。分割計画で各タスクに `kind` を与え(§7 の schema に
  ...
  **この 3 条件が防ぐのは「前回成果物の使い回し・タスク取り違え」という事故であって、偽装ではない**。
  ...
  epoch 形式の取り決めも `stat` の platform 差(`stat -f %m` / `stat -c %Y`)も書かずに済む。
  ただし比較の粒度差は残る(bash 3.2 は秒単位で、同一秒内の差は「新しくない」と判定される)。
  短く倒れる側が fail-closed なので実運用では問題にならない。
