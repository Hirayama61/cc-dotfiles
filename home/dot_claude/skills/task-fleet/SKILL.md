...
- **fan-out したブランチの Tier 3 所見は信用しない**。`scope-deviation-check.sh` は branch 版の
  ...
  見ないと気づけない形でフォールバックする**(`TIER3:` 行の `scope=` に pending 版の名前が出る)。fan-out した N 本のタスクブランチは同一 repo なので全て同じ pending を読み、それは案件
...
for wt in "${task_worktrees[@]}"; do
  ...
  tmp="$f.tmp.$$"
  printf 'fan-out from design-review of %s at %s\n' "$fleet_id" "$(date '+%Y-%m-%d %H:%M')" > "$tmp" \
...
- **タスク状態機械(schema)**: 各タスク `key` に次を持つ。
  ...
  - `artifact_paths`(そのタスクの期待成果物の**絶対パス**一覧。完了検知はここを見る。レポート
    本文から成果物パスを拾わない)/ `dispatched_at`(投入時刻。成果物 mtime の下限に使う)。
...
- **期待成果物はタスク種別ごとに定める**。分割計画で各タスクに `kind` を与え(§7 の schema に
  ...
  **成果物は「このタスク実行中に生成されたこと」まで確認する**。`report_path` / `artifact_paths` は
  どれも被運転が書き込めるパスで、判定が「実在し空でない」だけだと、行き詰まった被運転が 1 バイトの
  ファイルを置けば全チェックを通る(特に `review` / `qa` は成果物の非空だけが条件なので、レビュー
  タスクの「レビュー済み」偽陽性が統合まで通る)。次の 2 つを併せて課す。
  - **完了レポートに当該タスクの nonce を含めることを必須**にする(nonce は既に被運転へ渡っている)。
  - **`report_path` / `artifact_paths` の mtime が `dispatched_at`(schema)より新しいこと**を確認し、
    事前配置・使い回しを弾く。
  ここで機械的に言えるのは「このタスク実行中に生成された」までで、中身が妥当かは §8 の人間検品に残す。
...
- **失敗 pane は kill せず残す**(forensics)。ただし残すのは**スクロールバックだけ**で、被運転
  ...
  固定すればどちらでも安全側。tmux-claude-drive の retain と同一の契約)。
