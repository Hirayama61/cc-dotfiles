...
2. **被運転を起動**: `claude --model opus`(permission-mode 指定なし = defaultMode=auto)。
   ...
   **§1 の送信前 `PERMISSION_ERE` 照合はこの trust ダイアログにも一致する**(`do you trust` を
   ...
   それ以外の一致は例外なく人間へ上げる。例外が成立するのは「当該 pane でまだ被運転を起動して
   いない」**かつ**「一致箇所が `do you trust` である」の両方が揃った時だけで、片方でも欠けたら
   人間へ上げる(`PERMISSION_ERE` は `[y/n]` 等の他の形も拾うので、pane 状態だけで縛ると
   起動前 pane に残っていた別の一致まで例外に入る。2 条件とも機械条件なので、AND にしても
   機械ガードが文面判断に戻ることはない)。
...
- **期待成果物はタスク種別ごとに定める**。分割計画で各タスクに `kind` を与え(§7 の schema に
  ...
  **この 3 条件が防ぐのは「前回成果物の使い回し・タスク取り違え」という事故であって、偽装ではない**。
  ...
  mtime が完全一致した時だけ「新しくない」に倒れる(fail-closed 側)。
