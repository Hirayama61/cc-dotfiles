...
再実行は noclobber で止まって復旧できない。さらに scope はタスクごとに中身が違うので、
...
- **fan-out したブランチの Tier 3 所見は信用しない**。`scope-deviation-check.sh` は branch 版の
  `design-scope` が無い時、repo 単位の `design-scope-pending`(鮮度 24h)へ**黙ってフォールバック
  する**。fan-out した N 本のタスクブランチは同一 repo なので全て同じ pending を読み、それは案件
  全体の宣言スコープ = 個別タスクより必ず広い。結果は「所見が出ない」ではなく「広すぎる scope で
  照合されて誤った `OK` が出る」なので、SKIP より質が悪い。この状態のブランチで Tier 3 が `OK` でも
  スコープ照合を通過した根拠にはならない。
...
. "$HOME/.claude/hooks/lib/resolve-base-ref.sh" \
  || { echo "resolve-base-ref.sh を読めない。中断" >&2; exit 1; }
...
for wt in "${task_worktrees[@]}"; do
  ...
  # noclobber で atomic に作る(symlink/FIFO も O_CREAT|O_EXCL で弾ける)。上の -e は
  # メッセージ分けのための事前検査で、競合に対する実際の保護はこの set -C が担う。
  # 失敗時に消すのは「open できたが書込前に落ちた」空の残骸だけ。中身があるものは
  # 競合で他者が作った本物なので消さない(解錠キーを誤って削除しない)。
  ( set -C; printf 'fan-out from design-review of %s at %s\n' "$fleet_id" "$(date '+%Y-%m-%d %H:%M')" > "$f" ) \
    || { [ -s "$f" ] || rm -f "$f"; echo "フラグ作成に失敗: $branch。中断" >&2; exit 1; }
done
```
...
4. **指示を注入**: literal 指示に、**そのタスクのタスク指示書のパスだけ**を渡し(handoff doc 全文は
   ...
   管理セッションの完了検知が永久待ちになる。承認範囲の区別(**許可済みなのはこのタスク自体で、
...
- **期待成果物はタスク種別ごとに定める**。分割計画で各タスクに `kind` を与え(§7 の schema に
  ...
  どの種別でも**完了レポートは必須**で、`report_path` が実在し空でないことを確認する。
  ...
  git -C "$wt" diff --quiet "$base"..HEAD; rc=$?
  case "$rc" in
    0) committed=no ;;
    1) committed=yes ;;
    *) echo "diff 判定に失敗(rc=$rc): $wt。完了判定は保留し人間へ" >&2; exit 1 ;;
  esac
  # 未 commit の作業ツリー変更(tracked の変更 + untracked)も「変更あり」として拾う。
  dirty=no
  git -C "$wt" diff --quiet; [ $? -eq 1 ] && dirty=yes
  [ -n "$(git -C "$wt" ls-files --others --exclude-standard)" ] && dirty=yes
  ```
...
- **失敗 pane は kill せず残す**(forensics)。ただし残すのは**スクロールバックだけ**で、被運転
  ...
  pane 上限には数えない)。**順序は (1) capture → (2) 被運転プロセス終了 → (3) 失敗マークのリネーム
  に固定する**(tmux は pane のコマンドが終了すると既定で pane ごと消し、スクロールバックも失われる。
  先に終了させると保全対象が消える。tmux-claude-drive の retain と同一の契約)。
