...
- 材料は一時ファイルにまとめて reviewer へパスで渡す。state dir 配下に置くなら先に
  `"$HOME/.claude/hooks/lib/flag-paths.sh" dir-ensure` で dir を用意する
  (`$("$HOME/.claude/hooks/lib/flag-paths.sh" dir)/design-review-input-*` 等。bg job なら
  `$CLAUDE_JOB_DIR/tmp/` 配下でもよい)。

### 2. reviewer を 1 レスポンスで並列起動

- `Agent(subagent_type: "design-reviewer")` — 6 観点の反証レビュー
  (観点定義は agent 側が単一情報源。モデルは定義側で opus 固定)。
...
**提示の型は次の縮約例に従う**(概要ヘッダーで全体像 → 記録テーブルで決定を残す。
self-review 4a/4c の設計版。詳細な対話トリアージの作法は self-review を参照):

```text
## 設計レビュー結果サマリー
対象 Plan: {1 行タイトル} / 検査観点: 要件網羅・既存パターン整合・用語整合・スコープ妥当性・テスト方針・未検証前提
重大 {N} / 改善 {N} / 情報 {N}
reviewer: design-reviewer / Codex({実施 or skip 理由})

### 重大
| ID | 観点 | 対象(Plan の該当箇所) | 概要 | 対応 |
|---|---|---|---|---|
| F-001 | スコープ妥当性 | 手順3「既存 hook を改修」 | 影響範囲が宣言より広い | 今すぐ修正 |

### 改善 / 情報
| ID | 観点 | 対象 | 概要 | 対応 |
|---|---|---|---|---|
| F-002 | 用語整合 | 全体 | 「参謀ゲート」の語が未定義 | 見送る：CONTEXT.md に既出 |

### 総評
{VERDICT 行を転記}。実装着手の可否と、`今すぐ修正` があればその一覧。
```

指摘ゼロの severity はそのセクションを `(なし)` の 1 行にする。対話トリアージ
(1 問 1 件・`今すぐ修正`/`見送る`)は self-review 4b と同型で行う。
...
- branch フラグは repo+branch キーで `$XDG_STATE_HOME`(既定 `~/.local/state/claude-sessions`)に
  残る(worktree の破棄でも再起動でも消えない。dotfiles#49 B-3 で /tmp 共有名前空間から移行)。
  同名ブランチを切り直すと旧フラグで Gate が素通りしうるが、自分の Claude を縛る
  best-effort として受容する(気になる時は手で消す)。
