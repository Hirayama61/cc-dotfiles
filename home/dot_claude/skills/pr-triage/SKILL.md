...
- 各 finding の返信先へ返信した
  ただし手順 3 の打ち止め条件に該当した場合は、フラグ未作成・push 未了・返信未了のまま
  報告して終えるのが正常完了(この 3 つより打ち止めが優先)。
...
3. 全コミット後に `/self-review` を 1 回(push ゲート `pre-push-selfreview-gate` が
   ...
   **打ち止め条件**(self-review 手順 5 と同じ規則。これが無いと「直す → commit → フラグ失効 →
   再レビュー」で周回が終わらない。**対象は `/self-review` が返す finding だけ** — 手順 2 の
   方針表「対応」F-NNN は人間が確定済みなので判断語彙に関わらず全件実装する): 直すのは
   **判断が `必須`** の finding だけ(severity の `重大` とは別軸)。`推奨` 以下は直さず
   `triage:` へ記録する。軽微を直すための commit を作らない。4 周目でも `必須` が残るなら
   止めて人間へ報告する(周回上限は目安。この手順を実行するコールドセッションには周回数の
   出所が無いので、打ち止めを支えるのは `必須` だけ直すという主条件の側)。
   **打ち止めで止めた場合は手順 4・5 を実行せず**、push 未了・
   フラグ未作成・返信未了のまま報告して終えるのが正常完了(review-passed フラグを自分で
   作って push ゲートを解錠しない。返信もしない — 未 push の SHA を指す `対応コミット:` は
   PR 上で 404 リンクになる)。
