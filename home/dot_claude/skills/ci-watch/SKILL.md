...
**起動と同時に監視開始を 1 行宣言する** — bg 待機はゼロトークンで無言のため、宣言が無いと
「動いているのか」が人間に見えない。例: 「PR #<pr>(<repo>)の check 完了を bg 監視中(上限
<max×interval>=既定 15 分)。完了 or 上限で自動起床して評価する。」
...
threads="$(~/.claude/skills/ci-watch/scripts/fetch-coderabbit-threads.sh "$owner" "$name" "$pr")" \
  || { echo "ci-watch: CodeRabbit thread 取得に失敗(認証・ネットワーク・API)。手動再実行を促して打ち切る。" >&2; exit 1; }
```

- 出力が `IN_PROGRESS` なら「CodeRabbit レビュー進行中。数分後に再確認 or 手動 `/ci-watch $pr`」と
  伝えて打ち切る(CodeRabbit 専用の polling は張らない)。
- それ以外は未解決 CodeRabbit thread の JSON 配列(空なら `[]`)。`unresolved="$threads"` として
  手順 5 の delegate へ渡す。
- **取得経路だけを流用し `coderabbit:autofix` skill は起動しない**(適用フローを踏まない)。
  script が全ページを cursor pagination で回すので、SKILL.md 側に GraphQL を持たない。
...
**まず次のテンプレで全体像を締めてから**、判断が要るものだけ深掘りする(無言 bg 待機の後に
「何が分かったか」を一目で見せるため):

```text
## ci-watch 結果 — PR #<pr>(<repo>)
CodeRabbit 指摘 N 件: 妥当 A / 誤検知 B / 要ユーザー判断 C
失敗 check M 件:
| check | 分類(①修正/②テスト修正/③flaky/④見送り) | 一次判断の要点 |
|---|---|---|
| build (ci) | ① プロダクト修正 | 〜が未定義。<file>:<line> |
delegate 成果物: <Tasks/<repo>/ 配下の絶対パス>
```

- N=0 かつ M=0 なら「未解決 CodeRabbit thread も失敗 check も無い」と 1 行で締める
  (`NO_CHECKS` / `[]` の正常経路。表は省く)。
...
| CodeRabbit | **CLI 直叩き**(`coderabbit review`) | **PR 上の GitHub App thread** を共有 script で gh GraphQL 取得 |
