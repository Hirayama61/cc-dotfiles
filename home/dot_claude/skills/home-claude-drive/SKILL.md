...
description: >-
  ...
  task-fleet(現場監督の pane 並列。pane-claude-drive へ改名予定)の 3 兄弟の統括層。
user-invocable: true
...
- **被運転の再帰運転禁止を継承**: 現場監督はさらに home-claude-drive を起動しない。現場監督が
  pane 並列するときは task-fleet(pane-claude-drive へ改名予定)に従う。現場監督内部の通常の
...
- 右 pane にプレビューを起動する(トークンを消費しない素のシェルスクリプト。
  wt.sh と同じクロスリポ絶対パス参照)。**起動前に実体の存在を確認し、無ければ pane を
  作らず警告して続行する**(プレビューは表示面であり、状態把握は §6 の glob で成立する):

  ```bash
  preview="$HOME/ghq/github.com/Hirayama61/dotfiles/bin/fleet-preview.sh"
  if [ -x "$preview" ]; then
    preview_pane="$(tmux split-window -d -P -F '#{pane_id}' -h -t "$home_pane" "$preview")"
    case "$preview_pane" in %[0-9]*) ;; *) echo "プレビュー pane 生成に失敗" >&2 ;; esac
  else
    echo "fleet-preview.sh が無い(dotfiles 未 apply?)。プレビューなしで続行" >&2
  fi
  ```
