...
2. **指示を投入する(主経路 = `SendMessage`)**。被運転の peer 名は組み立てられない
   ...
   **この照合は 2 回目以降の投入でも毎回やり直す** — peer 名は同名解決が後発優先なので、
   ...
   見る。**起動直後の capture は空や描画途中を返すので、数秒待って再取得してから判定する**
   (1 回の空出力で「ダイアログではない」と判断しない)。行が現れない時は **trust ダイアログ
   待ちの可能性**があり、ダイアログなら前提 2 のとおり人間が押す。ダイアログでもなく行も
   現れないなら、fail-open 節の退避路(literal 送信)へ落とす。
