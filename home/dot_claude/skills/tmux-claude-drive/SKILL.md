...
2. **指示を投入する(主経路 = `SendMessage`)**。被運転の peer 名は
   `<cwd basename>-<ランダム2文字>` で**予測できない**ので、名前を組み立てず `ListAgents` から
   pane id で引き当てる:
   (i) `ListAgents` の各行が持つ pane id を手順 1 の `%NN` と照合し、**一意一致した行の名前だけ**を
   宛先にする。複数一致は同定不能なので送らず人間へ上げる。0 件なら (iv) の分岐へ。
   (ii) `SendMessage` で送る。**初回はその行の `[ref]` を付けた名前で再送する**(実測: 初回の
   bare name は拒否される)。2 回目以降は bare name でよい。
   (iii) **投入後に着手を確認する** — `ListAgents` の当該行が `idle` 以外へ動くか、
   `capture-pane` にメッセージの描画が出るかを見る。数分待って何も動かないなら放置せず
   pane 本文を人間へ見せる。
   (iv) **(i) が 0 件 / `ListAgents` 自体が使えない場合** はまず `capture-pane` で pane 本文を
   見る。起動直後に行が現れない時は **trust ダイアログ待ちの可能性**があり、ダイアログなら
   前提 2 のとおり人間が押す。ダイアログでもなく行も現れないなら、fail-open 節の退避路
   (literal 送信)へ落とす(退避路も送信前の権限プロンプト検知を先に通す。
   pane-claude-drive §1)。
   ...
   - 完了時の合図: 運転元が渡した**完了ファイルの絶対パス**へ nonce を書いて停止すること
     (パラメータ節)
   - **cron / loop を作らないこと**。残骸が勝手に再稼働するうえ、手順 5 の「閉じてよい」判定は
     `ListAgents` から cron の有無を見られないので、指示側で塞ぐしかない
3. **Monitor で監視**(sleep ポーリング禁止・完了/停止/失敗の全終端を拾う):
   45〜60 秒間隔で
   `tmux capture-pane -t "$pane_id" -p | grep -v '^[[:space:]]*$' | tail -25` を見て
   (空行を落としてから窓を取る理由は落とし穴)、
   (a) 成果物ファイルの出現 → 通知のみ、
   (b) `Do you want to proceed|❯ 1\. Yes` → 権限プロンプト(ユーザーへ)、
   (c) `API Error|usage limit` → エラー通知、(d) pane 消失 → 終了。
   フラグ変数で同一イベントの重複発報を抑止する。
   **完了検知に pane 本文を使わない** — 運転元が渡した完了ファイルの出現 + nonce 一致で判定し、
   あわせて `ListAgents` の当該 pane 行が `idle` であることを見る(入力 = pane・
   出力 = ファイルの分離)。pane 本文の字句照合は折り返しで指示文エコーが行末一致する・
   言い回し差で除外が効かない偽陽性が実測で出ており、スピナー不在との AND で補っても
   capture の窓の取り方に判定が依存し続ける。
4. 完了したら**現セッションが検品**する(成果物を通読し、規約・整合を突き合わせ、
   ...
   ```
5. **後片付け**: 畳む前に**閉じてよいか**を判定する。次の 4 条件すべてを満たす時だけ閉じ、
   1 つでも欠けたら閉じずに現状を人間へ上げる:
...
- `capture-pane -p | tail -N` は**末尾の空行に埋もれて何も返さないことがある**(実測)。
  空文字は `grep` を必ず不一致(rc=1)にするので、権限プロンプト検知のような fail-closed の
  判定では「一致しない = 送ってよい」に化ける。空行を落としてから窓を取り、非空行が 0 なら
  判定不能として止める(送信前ゲートの実装は pane-claude-drive §1)。
...
- **`SendMessage` / `ListAgents` が使えない環境では、従来の literal 送信へ落とす**。
  ...
  補間値の正規化は pane-claude-drive §5-3 が正典)。この経路では手順 2-(iii) の着手確認と
  手順 5 の状態条件が使えないので、着手は `capture-pane` で確認し、閉じてよい判定は
  完了ファイルと成果物条件だけで行う。状態条件が欠けた分は、閉じる前に人間が pane 本文を見る。
...
- **完了合図(nonce と完了ファイル)**: 運転元が pipeline/phase ごとに一意な nonce
