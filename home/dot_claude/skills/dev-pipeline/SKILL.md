...
- **redaction は決定論的パスで**(モデル遵守頼みにしない): pane スクロールバックには認証途中の値・
  環境変数ダンプが載りうる。書き出し前に既知トークン接頭辞(`sk-` / `ghp_` / `ghs_` / `xox` /
  `AKIA`、`Bearer` ヘッダ 等)と長い base64/hex 連続を ERE でマスクする 1 段を必須にし、その後に行数
  上限を掛ける。マスクをスクリプト化して呼ぶ(手作業で消させない)。<!-- follow-up: redaction スクリプトの実装 -->
