...
description: >-
  ...
  全体を回して」「パイプラインで実装まで通して」「dev-pipeline で」、`/dev-pipeline` での
  起動で発火する。
  1 パイプライン = 1 ブランチ = tmux window 群、1 フェーズ = 1 window
  (design=Phase 0 は指揮者が直接実行し window を作らない例外)。
user-invocable: true
...
- **allowlist(具体値)**: TOML から起動を許すのは次だけ。
  - `key` ∈ {`design`, `impl`, `qa`, `pr-polish`}(固定集合。他は拒否)。
  - `skill` ∈ {`grill-with-docs`, `design-review`, `fe-implement`, `fe-qa`, `self-review`, `ci-watch`}
    (差分/実装/レビュー系の既知 skill のみ。一致しない skill 名は起動しない)。
  - `gate` ∈ {`human`, `auto`}。`gate=auto` を許すのは `impl`→`qa` 遷移など**慣れた repo の
    非レビュー phase 間**だけで、`design`/`pr-polish`(hard gate を持つ)は auto にしても
    hard ゲート解除と権限プロンプト応答は人間(下記 hard 強制)。
- **文字列フィールドの決定論的検証**: `name` / `skill` / `key` は非空に加え、**改行・制御文字・
  シェルメタ文字を含む値を拒否**する(`[A-Za-z0-9._-]` 相当へ制限)。これらの値は被運転への
  literal 送信に補間されるため(§4-3)、改行混入は send-keys の途中確定(premature submit)+
  クロスセッション注入経路になる。allowlist 一致だけに頼らず形式で弾く。
- **allowlist は soft、hard 強制は別**: 上の allowlist 遵守は Claude が指示を守る model 実施の
  soft boundary。最終防波堤は次の hard 強制と初回人間承認。
...
3. **stray pending の掃除(条件付き)**: `design-reviewed-pending-<repo>`(branch 非依存)は
   別ブランチの別セッションが取得して誤解錠しうる。branch フラグが確定していれば本パイプラインに
   pending は不要なので掃除したいが、**pending は branch 非依存で所有者を区別できない**ため、
   無条件削除は同一 repo で並行する別パイプラインが直前に書いた pending まで消し、その
   ゲート解除を壊す競合になる。したがって掃除は **同一 repo で並行 dev-pipeline が無い時だけ**
   行う(単一パイプライン運転=既定の想定)。並行運転する時は掃除せず、各ブランチの branch
   フラグに委ねる(pending の漏れは既存の best-effort な残存リスクとして受容し、悪化させない)。
...
2. **被運転を起動**: `claude`(permission-mode 指定なし = defaultMode=auto に任せる。
   `--model` 省略 = 指揮者のデフォルトモデルを継承。フェーズ単位のモデル/effort 判定はしない)。
   起動確認時に pane の auto mode 表示も確認する(tmux-claude-drive 手順 1)。
3. **handoff doc を注入**: 被運転への literal 指示に、handoff doc のパスと「これだけ読んで当該
   フェーズの合成 skill(定義の `skill`)を回せ」「完了時に nonce を完了ファイル
   `Tasks/<repo>/.done-<pipeline>-<phase>` に書いて停止」を含める。**補間する値(pipeline-id・
   nonce・handoff パス・skill 名)は単一行・制御文字なしを保証してから送る**(改行が literal 送信に
   混じると途中確定 + 別入力送信になる。pipeline-id/nonce は `[A-Za-z0-9._-]` へ正規化する)。
4. **Monitor で監視**(tmux-claude-drive 手順 3): 完了ファイルの出現 / 成果物出現 / 権限プロンプト /
   API Error・usage limit / pane 消失。usage limit を検知したら §6 経路 A のタイマーへ委譲する。
   完了検知は pane 本文でなく完了ファイルで行う(§7 の入力/出力分離)。
5. **検品**: 完了ファイルの nonce 一致を確認してから、指揮者が成果物を通読・規約整合を検品する。フェーズ内の
...
`auto` なら次フェーズへ。**未宣言は human**。`auto` の意味は「**遷移判断だけ無人**」で、
被運転の権限プロンプト応答は auto でも人間が行う。hard ゲート
(design-reviewed / review-passed)と権限プロンプトは gate 宣言に依らず常に人間。

**各フェーズの開始と遷移は 1 行宣言する** — window が分かれて進捗が指揮者 pane に散るため、
今どのフェーズかを明示しないと人間が全体像を追えない。フェーズ開始時に
「Phase <key>(window <name>)開始」、検品通過時に「<key> 完了 → 次 <key> へ」
(auto)/「<key> 完了 → 承認待ち」(human)を出す。

**パイプライン完了時(終端フェーズ通過後)は次の型で締める** — 各フェーズが別 window・別
コンテキストで走った結果を、観測点である指揮者が 1 枚に集約して人間へ返す:

```text
## dev-pipeline 完了 — <pipeline-id>(<repo> / branch <branch>)
| phase | 結果 | 成果物・要点 |
|---|---|---|
| design | done | design-reviewed フラグ済。<Plan/ADR パス> |
| impl | done | review-passed。<主要変更の要点> |
| qa | done | <検出バグ N 件 → 全解消 / 残 M 件> |
| pr-polish | done | PR #<n> push 済。<URL> |
handoff doc: <Tasks/<repo>/<pipeline-id>-handoff.md>
要判断・残件: <人間の判断が要る残件。無ければ「なし」>
```

失敗・中断で終わった場合は当該 phase を `failed`/`aborted` で示し、§6 の再開経路(A/B)と
forensics ログ(§8)のパスを添える。
...
- **nonce と完了検知(入力/出力の分離)**: pipeline+phase ごとに指揮者が起動時に一意採番
  (例 `<pipeline>-<phase>-<連番>`)。**完了検知は pane 本文の grep でなく専用の完了ファイルで
  行う** — 指示文自体に nonce を含めて pane を検索すると、送信直後の指示エコーを完了と誤検知する
  (一意性では自己反射を防げない)。被運転には「完了時に nonce を**完了ファイル
  `Tasks/<repo>/.done-<pipeline>-<phase>` に書いて**停止」と指示し、指揮者はその**ファイルの出現 +
  中身の nonce 一致**で done とする(入力=pane、出力=ファイルで経路を分ける)。nonce は再利用しない。
...
- **失敗時**(API Error / usage limit / 検品 NG)は window を kill せず生かす。`capture-pane -S`
  ...
  秘匿)を掛け、**redaction 済みの末尾 N 行を別ログパス**へ、handoff には要約を書く(全文平文は
  残さない)。証跡がファイルに残るのでセッション終了でも失われない。
- **redaction は決定論的パスで**(モデル遵守頼みにしない): pane スクロールバックには認証途中の値・
  環境変数ダンプが載りうる。書き出し前に必ず **`scripts/redact-forensics.sh` を通す** ―
  既知トークン接頭辞(`sk-` / `ghp_` / `ghs_` / `xox` / `AKIA` / `AIza`、`Bearer`・`Authorization`
  ヘッダ 等)と長い base64/hex 連続を ERE でマスクし、末尾 N 行に切り詰める(過剰マスク寄り=安全側)。
  例(先に 0600 で作成してから追記):
  `install -m 600 /dev/null <forensics.log>` →
  `tmux capture-pane -J -p -S -2000 -t <win> | <skills>/dev-pipeline/scripts/redact-forensics.sh 500 >> <forensics.log>`。
  **`-J` は必須**(折り返しで複数行に分割されたトークンを redaction が取りこぼさないよう結合する)。
  redaction は best-effort(短い/未知形式の秘密は残りうる)なので、`<forensics.log>` は
  **0600・非共有ディレクトリ**(例 `Tasks/<repo>/` 配下)に置く(`install -m 600` か `umask 077` で
  world-readable を避ける)。手作業で消さない(モデルの見落としで秘密が永続化するのを防ぐ)。
