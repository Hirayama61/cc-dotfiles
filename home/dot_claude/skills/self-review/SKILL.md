...
1. **対象特定**: `git branch --show-current` で現在ブランチを確認。base ref
   を次の順で一意に決める: ①upstream 追跡があれば `@{u}` ②無ければ存在する
   方の `main`/`develop` ③現在ブランチ自体が保護ブランチ(main/develop/epic
   /**)で比較先が無い場合は `HEAD~1`。`git diff <base ref>...HEAD`(三点
   記法。共通祖先からの差分を git が内部算出するので `git merge-base` を別途
   叩かない=保護ブランチ系 hook と文字列衝突しない)で変更全体を把握。③に
   該当する場合は「保護ブランチ上」を**このスキルでは解禁不可**の確定条件と
   して扱う(検査は通常どおり実施・報告し、フラグだけ立てない)。
...
4. **判定**:
   ...
   - **指摘あり** → フラグを**書かない**。自分で直せるものは直し、再レビュー。
     ...
     エスカレーションする(勝手な判断をしない)。既に commit 済みの
     secret/機密は履歴改変(破壊的・human-judgment)を要するため自己修正
     対象外、常にエスカレーション(working tree 修正と違い自己完結しない)。
   - **フラグ対象の同一性**: 「指摘ゼロ」は *commit 済みの現 HEAD* が clean の
     意味。自己修正した場合は修正を commit して初めてフラグ可(commit で旧
     フラグは無効化されるので新 HEAD を再レビューしてから立てる)。commit が
     自分のスコープ外なら、修正済みでもフラグは立てず内容を提示して
     エスカレーション(未 commit の working tree にフラグを立てない)。
   - 解禁不可条件(保護ブランチ上 / secret / memory 違反 等)は OR 評価。
     1つでも成立すればフラグ不可。優先順位は不要、成立した理由は全て併記。
