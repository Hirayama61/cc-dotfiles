...
- フラグの書き方は、4b で集めた Tier ack 理由 + 見送り処置を内容に記録する
  ...
  - 該当ゼロ(指摘ゼロ かつ Tier OK/SKIP)なら内容は空でよい(空ファイルを作る)。
    ...
    # 内容(tier1-ack / tier2-ack / 見送り triage)の実書込は次手順のフラグ作成
    # スニペットが単一の atomic create でまとめて行う。ここでは reason の検証のみ。
    ```
- トリアージ完了を確認したらフラグを立てる(キーは `flag-paths.sh` が単一情報源。
  ...
  **このスキルはレビュー対象 repo 内(`$PWD` = 対象 worktree)で実行すること**。gate /
  ...
  # 二次網: 子 reviewer の read-only 起動(手順 2b)が一次防御。ここで既存なら異常
  # (子や別経路の自力作成を疑う)として中断する。後段の noclobber 作成失敗とは別メッセージに
  # して、「既存=異常シグナル」を「権限/state dir 等の作成失敗」と切り分ける。
  [ -e "$flag" ] && { echo "想定外: review-passed フラグが既存(子/別経路の自力作成を疑う)。中断" >&2; exit 1; }
  # 作成は noclobber(`set -C`)下の単一の `>` で行う。上の `[ -e ]` 後の窓に別プロセスが
  # 割り込んでも、既存なら `>` が失敗して中断する(TOCTOU を塞ぐ)。内容書込・空作成を同一経路で
  # 扱い、ack / 見送り理由(前手順の reason1 / reason2 + 見送り triage)があれば内容を、無ければ
  # 空を書く(gate は `-f` 存在のみ参照で互換):
  ( set -C
    {
      if [ -n "$reason1" ]; then printf 'tier1-ack: %s\n' "$reason1"; fi
      if [ -n "$reason2" ]; then printf 'tier2-ack: %s\n' "$reason2"; fi
      # 見送った finding を 1 行 1 件(例: triage: F-002 見送り — 誤検知(...))。該当ゼロなら空。
    } > "$flag"
  ) 2>/dev/null \
    || { echo "review-passed フラグの作成に失敗(権限/state dir/競合)。中断" >&2; exit 1; }
  ```
