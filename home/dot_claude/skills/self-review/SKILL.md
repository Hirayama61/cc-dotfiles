...
- フラグの書き方は、4b で集めた Tier ack 理由 + 見送り処置を内容に記録する
  ...
  - 該当ゼロ(指摘ゼロ かつ Tier OK/SKIP)なら内容は空でよい(空ファイルを作る)。
    ...
    # noclobber 下で内容書込も atomic に(既存なら `>` が失敗 = TOCTOU 窓を塞ぐ)
    ( set -C
      {
        if [ -n "$reason1" ]; then printf 'tier1-ack: %s\n' "$reason1"; fi
        if [ -n "$reason2" ]; then printf 'tier2-ack: %s\n' "$reason2"; fi
        # 見送った finding を 1 行 1 件で記録(例: triage: F-002 見送り — 誤検知(...))
        # 見送りゼロなら何も出力しない
      } > "$flag"
    ) 2>/dev/null \
      || { echo "想定外: review-passed フラグが既存(または作成失敗)。中断" >&2; exit 1; }
    ```
- トリアージ完了を確認したらフラグを立てる(キーは `flag-paths.sh` が単一情報源。
  ...
  **このスキルはレビュー対象 repo 内(`$PWD` = 対象 worktree)で実行すること**。gate /
  ...
  # 二重の安全網: 子 reviewer は read-only 起動(手順 2b)でフラグ作成を封じる一次防御。
  # これはその二次網で、フラグが既存なら異常として中断する(子や別経路の自力作成検知)。
  "$HOME/.claude/hooks/lib/flag-paths.sh" dir-ensure \
    || { echo "flag state dir の検証に失敗。中断" >&2; exit 1; }
  # 存在チェックと作成を atomic に(noclobber: 既存なら `>` が失敗 = TOCTOU 窓を塞ぐ)。
  # ack / 見送り理由があれば上の `{ ... }` 内容を、無ければ空を書く(いずれも `set -C` 下):
  ( set -C; : > "$flag" ) 2>/dev/null \
    || { echo "想定外: review-passed フラグが既存(または作成失敗)。中断" >&2; exit 1; }
  ```
