...
- **委譲先モデルは難易度でルーティング**: delegate 起動時に `model` を明示する
  (検索・読込のみ=`haiku` / 機械的編集・定型調査=`sonnet` / 設計判断・難デバッグ・
  実装中核=`opus`)。delegate 定義の既定は `sonnet`(安全網)で、メインが易タスクは
  `haiku` へ・難タスクは `opus` へ起動時に上書きする。判断はメインが下す。
