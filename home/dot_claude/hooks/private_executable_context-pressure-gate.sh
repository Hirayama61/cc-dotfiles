...
#   - state file と決定ログへの Write は素通し(cache 配下を一律に通すと、鮮度スタンプ等
#     ゲート自身の判断材料まで編集系ツールで書けてしまうため 2 パスに限定する)
...
type claude_ctx_usage_stale_secs >/dev/null 2>&1 || exit 0
...
# 脱出経路は state file と決定ログの 2 パスだけ通す(下の deny 文が宣言している範囲)。
# cache 配下を一律に通すと、鮮度スタンプや打ち止めのような
# ゲート自身の判断材料まで編集系ツールで書けてしまう。
fp="$(hook_field '.tool_input.file_path // .tool_input.notebook_path')"
case "$fp" in
"$(ctx_state_file "$ctx")" | "$(ctx_decisions_file "$ctx")") exit 0 ;;
esac
