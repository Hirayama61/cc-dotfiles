...
# stuck-nudge.sh の種別ごとカウンタと 1 ctx 1 回 claim(ディレクトリ)を破棄する
# (clear|compact 後の新しい文脈では詰まり検知をやり直す)。版ずれは無視。
if type stuck_count_flag_prefix >/dev/null 2>&1; then
  rm -f "$(stuck_count_flag_prefix "$ctx")"* 2>/dev/null || true
  rm -rf "$(stuck_nudged_flag "$ctx")" 2>/dev/null || true
fi
# delegation-nudge.sh の探索累計カウンタと claim も同時に再武装する。版ずれは無視。
if type delegation_count_flag >/dev/null 2>&1; then
  rm -f "$(delegation_count_flag "$ctx")" 2>/dev/null || true
