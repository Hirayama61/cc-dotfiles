...
#
# best-effort な抑止であり完全防御ではない: alias / シェル関数 / クォート分断 /
# 変数置換(例 g(){ gh "$@";}; g pr merge)・`command gh` / `xargs gh` 等のラッパー
# 経由・`{ gh ...; }` やバッククォートでの起動は grep の字句検査では原理的に捕捉
# できない。あくまで「うっかり実行」の抑止(既存 hook 群と同じ性質)。
...
# 直後(&& || | ( $( の境界を単一文字でカバー)+ 任意空白 + 任意の環境変数前置。
# これで文字列リテラルやコメント内の gh 言及を誤ブロックしない。チェインは BORDER が
# 各 gh の直前に来るので維持される。
# 末尾 END はサブコマンド名の直後の境界: 空白 / 行末に加え、シェル区切り ; & | ) も
# 終端として許容する(前方 BORDER と対称)。これで `$(gh pr merge)`・`(gh pr merge)`・
# `gh pr merge; ...`・`gh pr merge|cat`・`gh pr merge&` の取りこぼしを塞ぎつつ、
# サブコマンド名の prefix(create が createX 等)や read-only の誤ブロックは防ぐ。
BORDER='(^|[;&|(])[[:space:]]*'
END='(\s|$|[;&|)])'
if echo "$cmd" | grep -qE "${BORDER}${ENV}gh\\s+${FLAGS}pr\\s+(ready|merge|close|reopen)${END}"; then
  echo "ブロック: gh pr の ready/merge/close/reopen は不可逆な PR 状態変更のため禁止。人間が判断・実行すること。" >&2
  exit 2
fi

if echo "$cmd" | grep -qE "${BORDER}${ENV}gh\\s+${FLAGS}release\\s+(create|delete|edit|upload)${END}"; then
  echo "ブロック: gh release の create/delete/edit/upload は公開リリースを動かすため禁止。人間が判断・実行すること。" >&2
  exit 2
fi

if echo "$cmd" | grep -qE "${BORDER}${ENV}gh\\s+${FLAGS}repo\\s+(delete|archive|edit)${END}"; then
