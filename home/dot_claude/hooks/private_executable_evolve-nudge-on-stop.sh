...
[[ -n "$ctx" ]] || exit 0
# ctx は外部入力由来で glob 展開(下の cs_flags)に使うため、メタ文字を含む値は素通り
# (別 ctx のフラグへの誤マッチを防ぐ。正規の transcript basename は必ず通る)。
...
# 1 ctx 1 回の mark を出力より先に確定する。state dir 書込不能のまま出力すると停止の
# たびに block が反復する(ナッジの欠落より反復の方が有害)。mark 後に jq が失敗して
# ナッジが失われるのは受容(次の clear/compact の rearm で再武装される)。
{ claude_flag_dir_ensure && touch "$(evolve_nudged_flag "$ctx")"; } 2>/dev/null || exit 0
