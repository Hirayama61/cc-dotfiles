...
echo "$cmd" | grep -qE '\bgit\s+(push|merge)\b' || exit 0
...
if echo "$cmd" | grep -qE '\bgit\s+push\b' && is_protected "$branch"; then
  echo "ブロック: 保護ブランチ(${branch})への push は禁止。feature ブランチから人間が PR を作成すること。" >&2
  exit 2
fi

# merge: 保護ブランチ上での merge(= 保護ブランチへのマージ)を拒否
if echo "$cmd" | grep -qE '\bgit\s+merge\b' && is_protected "$branch"; then
