...
- **このリポは PUBLIC**。ゲート類(push ゲート・設計レビューゲート・消失検知)について、
  **何が保証されないか**は書く(誤読すると防御を過大評価するため)。書かないのは
  **正規の手順を迂回する方法** — 検証を通さずにフラグを成立させる手口や、どの既存 agent が
  その能力を持つか。SKILL.md / hook ヘッダ / テスト名 / PR 本文のいずれも同じ。
  必要になったら hook と agent 定義を読めば分かるので、説明として書き足さない。
  **正規の手順そのものの仕様**(スクリプトに渡す入力の書式・語彙・受理条件)は利用者が
  要るので通常どおり書く。禁じているのは迂回路の説明であって、正面の入口の説明ではない。
...
- **lib は共有判定の単一情報源**(キー・判定の完全一致が生命線。インライン複製を
  ...
  | `flag-paths.sh` | ゲートフラグの state dir(`${XDG_STATE_HOME:-$HOME/.local/state}/claude-sessions`)提供と全キー導出 + review-passed の `head:` 行書式(生成・解釈) |
  ...
  | `context-paths.sh` | コンテキスト逼迫対策(context-pressure)の cache パス(`~/.cache/claude-context/<ctx>/`)と ctx キー導出 + state-stamp の行書式(生成・解釈)と state file の鮮度判定(閾値含む) |
